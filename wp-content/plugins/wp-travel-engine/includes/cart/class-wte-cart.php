...
class WTE_Cart {
	...
	function get_total( $with_discount = true ) {
		...
		$get_total = array(
			'cart_total'         => floatval( $cart_total ), // Effective for multiple cart items[cart_total].
			'discount'           => floatval( $discount_amount ),
			'sub_total'          => floatval( $total_trip_price_after_dis ),
			'total'              => floatval( $total_trip_price ),
			'trip_extras_total'  => floatval( $trip_extras_total ),

			// Partial payments.
			'cart_total_partial' => floatval( $cart_total_partial ),
			'discount_partial'   => floatval( $discount_amount_partial ),
			'sub_total_partial'  => floatval( $total_trip_price_partial_after_dis ),
			// 'total_partial'      => floatval( $total_trip_price_partial ),
			'total_partial'      => number_format( $cart_total_partial, 2, '.', '' ),
		);
	...
	public function get_cart_item_id( $trip_id, $pricing_id = '', $start_date = '', $time = '' ) {
		$datetime_instance = new DateTime( $start_date . ' ' . $time );
		$suffix       = $datetime_instance->format( 'dmYHi' );
