...
if( ! class_exists( 'Yala_Travel_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Yala_Travel_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__( 'row', 'yala-travel' ),
                'field' => false,
            );
            ...
            if ( empty( $this->button_label ) ) {
                ...
                $this->button_label = sprintf( esc_attr__( 'Add new %s', 'yala-travel' ), $this->row_label['value'] );
            }
        ...
        public function enqueue() {    
            ...
            wp_enqueue_script( 'yala-travel-repeater', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'yala-travel-repeater', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                ...
                printf( esc_attr__( 'Limit: %s rows', 'yala-travel' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
            ...
            <script type="text/html" class="customize-control-repeater-content">
                ...
                <li class="repeater-row minimized" data-row="{{{ index }}}">
                    ...
                    <div class="repeater-row-content">
                        <# _.each( data, function( field, i ) { #>
                            ...
                            <div class="repeater-field repeater-field-{{{ field.type }}}">
                                ...
                                <# } else if ( 'color' === field.type ) { #>
                                    ...
                                    <label>
                                        ...
                                        <# if ( field.description ) { #>
                                            <span class="description customize-control-description">{{{ field.description }}}</span>
                                        <# } #>
                                        <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'yala-travel' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                                ...
                                <# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
                                    ...
                                    <figure class="yala-travel-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'yala-travel' ); ?>" >
                                        <# if ( field.default ) { #>
                                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                                            <img src="{{{ defaultImageURL }}}">
                                        <# } else { #>
                                            <?php esc_attr_e( 'No Image Selected', 'yala-travel' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'yala-travel' ); ?></button>
                                        <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'yala-travel' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'yala-travel' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change Image', 'yala-travel' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add Image', 'yala-travel' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                ...
                                <# } else if ( field.type === 'upload' ) { #>
                                    ...
                                    <figure class="yala-travel-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'yala-travel' ); ?>" >
                                        <# if ( field.default ) { #>
                                        ...
                                        <# } else { #>
                                            <?php esc_attr_e( 'No File Selected', 'yala-travel' ); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                                        <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'yala-travel' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'yala-travel' ); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e( 'Change File', 'yala-travel' ); ?>
                                            <# } else { #>
                                                <?php esc_attr_e( 'Add File', 'yala-travel' ); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                        ...
                        <# }); #>
                        <button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'yala-travel' ); ?></button>
                    </div>
                </li>
            </script>
            <?php
